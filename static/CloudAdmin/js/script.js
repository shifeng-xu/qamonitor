var App = function () {
	...
	var handleMagicSuggest = function (listdata) {
		var jsonData = [];
			//列表
		//var response_data = {{ response_data|safe }};
    	//var cities=response_data['proList']
        //var cities = 'New York,Los Angeles,Chicago,Houston,Paris,Marseille,Toulouse,Lyon,Bordeaux,Philadelphia,Phoenix,San Antonio,San Diego,Dallas,San Jose,Jacksonville'.split(',');
            ...
            for(var i=0;i<listdata.length;i++) jsonData.push({id:i,name:listdata[i],status:'true',coolness:Math.floor(Math.random() * 10) + 1});
	...
	return {
        ...
        init: function () {
            ...
            if (App.isPage("bugsoffline")) {
				handleBootbox(); //Function to display Date Timepicker
				//handleMagicSuggest(cities); //Function to display autosuggest
				//handleDateColorpicker(); //Function to handle date and time picker
		...
		initBugOffline:function (proList) {
			handleMagicSuggest(proList);
			handleDateTimePickers(); //Function to display Date Timepicker
			handleSelect2(); //Function to handle select2
        },
